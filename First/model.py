...
def layer_3(x):
    ...
    conv1_2 = tf.layers.conv2d(pool1_1, 1, 5, padding='same')
    pool1_2 = tf.layers.max_pooling2d(conv1_2, pool_size=2, strides=1, padding='valid')
    conv1_3 = tf.layers.conv2d(pool1_2, 1, 3, padding='same')
    pool1_3 = tf.layers.max_pooling2d(conv1_3, pool_size=2, strides=1, padding='valid')
    conv1_4 = tf.layers.conv2d(pool1_3, 1, 3, padding='same')
    conv1_5 = tf.layers.conv2d(conv1_4, 1, 3, padding='same')

    fc = tf.contrib.layers.flatten(conv1_5)
...
def layer_2(x):
    ...
    fc = tf.contrib.layers.flatten(conv1_4)
    logits = tf.layers.dense(fc, 3)
    return logits

def layer_1(x):
    conv1_1 = tf.layers.conv2d(x, 1, 3, padding='same')
    pool1_1 = tf.layers.max_pooling2d(conv1_1, pool_size=2, strides=1, padding='valid')
    conv1_2 = tf.layers.conv2d(pool1_1, 1, 3, padding='same')
    pool1_2 = tf.layers.max_pooling2d(conv1_2, pool_size=2, strides=1, padding='valid')
    conv1_3 = tf.layers.conv2d(pool1_2, 1, 3, padding='same')

    fc = tf.contrib.layers.flatten(conv1_3)
    logits = tf.layers.dense(fc, 3)
    return logits
