...
6. 请确认left、right accuracy>85%

以下为建议做，可不做(自己衡量):
7. 尝试不同 zero_label 的值(在 DataManager 里，可由 main 修改)，看看精度会如何变化，建议尝试值：3、5、8、10。
...
### :point_right: 本周计画：
deadline(~12/20)<br>
下一次会是最后一次聚会，之后可能就deadline前整理一下报告而已。 <br>
我已经把报告写好八成了，基本上就剩一些实验数据需要填进去了。 <br>
以下列出我们还缺少的部分：

```
1. 最终实验精度(abstract)
2. 第一层最终架构、精度、实验数据、跟OpenFace比较结果(3.2、5.1)
3. 第二层最终架构、精度、实验数据(3.3、5.2)
4. 组内分工
```
